...
class possion():
    ...
    def __init__(self, prepicPath, bakpicPath, cp2pt, zone_width=20, save_abs_path=""):
        """
        prepic: 前景图片
        bakpic: 背景图片
        zone_width: 带状区域的宽度，值越大，生成的效果会越好
        """
        self.save_abs_path = save_abs_path
        self.prepic = cv2.imread(prepicPath)
        self.bakpic = cv2.imread(bakpicPath)
    ...
    def construct_mat(self):
        for j in range(self.conAfromH, self.bh):
            for i in range(self.conAfromW, self.bw):
                ...
                if horn != -1:
                    ...
                    self.__sparse_mat_set_v__(
                        idx, self.matAw * (j - self.conAfromH) + (i - self.conAfromW) + horn[1], -1)
                    self.__sparse_mat_set_v__(
                        idx, self.matAw * (j - self.conAfromH + horn[0]) + (i - self.conAfromW), -1)
                    ...
                    if bdy != -1:
                        ...
                        for item in bdy[0]:
                            self.__sparse_mat_set_v__(
                                idx, self.matAw * (j - self.conAfromH + item) + i - self.conAfromW, -1)
                        for item in bdy[1]:
                            self.__sparse_mat_set_v__(
                                idx, self.matAw * (j - self.conAfromW) + i - self.conAfromW + item, -1)

                        isina = self.__isinarea__((j, i))

                        if isina == 1:
                            tmp1 = list((j + item - self.cp2h, i - self.cp2w)
                                        for item in bdy[0])
                            tmp2 = list((j - self.cp2h, i - self.cp2w + item)
                                        for item in bdy[1])

                            self.__b_mat_set_v__(
                                self.prepic, (j - self.cp2h, i - self.cp2w), tmp1 + tmp2)
                        else:
                            tmp1 = list((j + item, i) for item in bdy[0])
                            tmp2 = list((j, i + item) for item in bdy[1])

                            self.__b_mat_set_v__(
                                self.bakpic, (j, i), tmp1 + tmp2)
                    else:
                        #print(debug_info(), "inner", j, i)
                        self.__sparse_mat_set_v__(idx, idx, 4)
                        self.__sparse_mat_set_v__(
                            idx, self.matAw * (j - self.conAfromH) + i - self.conAfromW - 1, -1)
                        self.__sparse_mat_set_v__(
                            idx, self.matAw * (j - self.conAfromH) + i - self.conAfromW + 1, -1)
                        self.__sparse_mat_set_v__(
                            idx, self.matAw * (j - self.conAfromH - 1) + i - self.conAfromW, -1)
                        self.__sparse_mat_set_v__(
                            idx, self.matAw * (j - self.conAfromH + 1) + i - self.conAfromW, -1)
        ...
        for pt in self.bdylist:
            self.__sparse_mat_set_v__(index + self.matAw * self.matAh,
                                      (pt[0] - self.conAfromH) * self.matAw + pt[1] - self.conAfromW, 1)
            index += 1
        ...
        shape = (self.matAw * self.matAh +
                 len(self.bdylist), self.matAh * self.matAw)
    ...
    def calc_x(self):
        ...
        tmp0 = lsmr(self.matA, self.matb[0],
                    show=False, atol=1.0e-8, btol=1.0e-8)
        tmp1 = lsmr(self.matA, self.matb[1],
                    show=False, atol=1.0e-8, btol=1.0e-8)
        tmp2 = lsmr(self.matA, self.matb[2],
                    show=False, atol=1.0e-8, btol=1.0e-8)
        ...
        for j in range(self.conAfromH, self.bh):
            for i in range(self.conAfromW, self.bw):
                self.bakpic[j, i] = (
                    res[(j - self.conAfromH) *
                        self.matAw + i - self.conAfromW][0],
                    res[(j - self.conAfromH) *
                        self.matAw + i - self.conAfromW][1],
                    res[(j - self.conAfromH) *
                        self.matAw + i - self.conAfromW][2],
                )
